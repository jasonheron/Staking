...
class AbstractTransport extends typedEventEmitter_1.TypedEmitter {
    ...
    getDiff(nextDescriptors) {
        const connected = nextDescriptors.filter(nextDescriptor => !this.descriptors.find(descriptor => descriptor.path === nextDescriptor.path));
        const disconnected = this.descriptors.filter(d => nextDescriptors.find(x => x.path === d.path) === undefined);
